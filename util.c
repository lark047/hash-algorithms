...
uint32_t append_padding(uint8_t **digest_ref, const char *msg, uint32_t *length)
{
    ...
    /* +1 for 0x80 */
    while (((*length + 1) * CHAR_BIT + nils) % DIGEST_LENGTH_BITS != PADDED_LENGTH_BITS)
    {
        ++nils;
    }
    ...
    const uint32_t total_length = *length + 1 + nils + BLOCK_SIZE_BITS / CHAR_BIT;
    const uint32_t block_count = total_length / BLOCK_SIZE_BITS;
    PRINT("total_length / 64 = %u (%u) <== should be 0\n", block_count, total_length % BLOCK_SIZE_BITS);
