...
int main(int argc, char **argv)
{
    ...
    if (argc == 2)
    {
        if (strcmp(argv[1], "-t") == 0)
        {
            ...
            {
                if ((suite = CU_add_suite("SHA256 Test Suite", NULL, NULL)) == NULL)
                {
                    CU_cleanup_registry();
                    rc = CU_get_error();
                }
                ...
                {
                    if (CU_ADD_TEST(suite, testSHA256) == NULL)
                    {
                        CU_cleanup_registry();
                        rc = CU_get_error();
                    }
        ...
        else
        {
            ...
            uint8_t *digest = SHA256string(msg);
    ...
    else
    {
        ...
        puts("  prints the SHA256 hash of <string>\n");
