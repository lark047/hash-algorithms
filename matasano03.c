...
static double CalculateScore(const uint8_t * const, const uint64_t);
...
double CalculateScore(const uint8_t * const xor_result, const uint64_t length)
{
    ...
    /**
     * Should this be
     * const double hit_score = (double) text.length / text.hits;
     */
    const double hit_score = (double) (ASIZE - text.hits + 1) / ASIZE; /* => 27 - uint64_t ??? Why is this ASIZE - text.hits? */
