...
uint8_t *SHA512_with_initial_values(const uint8_t *, uint64_t, const uint64_t *);
...
uint8_t *SHA512(const uint8_t *msg, uint64_t msg_length)
{
    ...
    return SHA512_with_initial_values(msg, msg_length, H0);
}

uint8_t *SHA512_with_initial_values(const uint8_t *msg, uint64_t msg_length, const uint64_t *H0)
{
    ...
    PRINT("Copying %u * %u = %u bytes to H[0..7]\n", SIZE(H), sizeof *H, SIZE(H) * sizeof *H);
    memcpy(H, H0, SIZE(H) * sizeof *H);
